...
export class BattleManager {
  private battles: Map<UID, BattleState> = new Map();
  private playerToBattle: Map<UID, UID> = new Map();
  ...
  processGesture(payload: GesturePayload): BattleState | null {
    ...
    if (!battle || battle.status !== BattleStatus.IN_PROGRESS) return null;

    const player = this.getPlayerInBattle(battle, payload.playerId);
    if (!player || player.isStunned) return null;
  ...
  castJutsu(payload: JutsuCastPayload): JutsuResultPayload | null {
    ...
    const result: JutsuResultPayload = {
      jutsuId: jutsu.id,
      casterId: caster.id,
      targetId: target.id,
      damage,
  ...
  updateBattle(battleId: UID): BattleState | null {
    ...
    if (elapsed >= battle.duration) {
      const winner =
        battle.player1.hp > battle.player2.hp
          ? battle.player1.id
  ...
  endBattle(
    ...
    const player1Result: BattlePlayerResult = {
    ...
    const player2Result: BattlePlayerResult = {
  ...
  private getPlayerInBattle(battle: BattleState, playerId: UID): PlayerBattleState | null {
